...
- **進貨** — register a purchase: date, model, total amount, one 貨號 (serial) per machine, with an 一般庫存/試用機 toggle and auto-serial generator; the edit modal lists every machine of the purchase with in-place serial editing (atomic, swap-safe, synced into sale history)
